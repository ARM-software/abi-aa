...
-  With some exceptions clearly identified by the C language standard
   (for example, whether malloc(0) :math:`\ne` NULL), Its behavior is fixed by the
   C language standard.
...
in `clibabi32-table1`_ below. Some are defined by the ANSI 1989 (ISO
...
eventually call a function of 3 arguments as shown in `clibabi32-table3`_, below. As
specified by the C standard, this function must print details of the
failing diagnostic then terminate by calling abort. A C library
implementation can fabricate a lighter weight, no arguments,
non-printing, non-conformant version of assert() by calling abort
directly, so we define no variants of \_\_aeabi\_assert().
...
We define it as shown in `clibabi32-table4`_, below.
...
Locale.h defines 6 macros for controlling constants (`clibabi32-table8`_) and struct
...
column of `clibabi32-table9`_ can use the same definitions inline when
...
out of line. If Branch Target Identification mechanism is enabled, the
longjmp may elect to restore the environment, saved in the jmp\_buf, in
a way that requires the location, where the execution continues, to be
identified as a valid destination for indirect jumps.
...
It is a quality of implementation whether at least one byte of program
image space will be allocated to each of \_\_aeabi\_SIG\_\* listed in
`clibabi32-table11`_, or whether references to those values will be relocated to
distinct, target-dependent constants.

Signal.h defines six SIGXXX macros. We recommend the common Linux/Unix
values listed in `clibabi32-table12`_. All signal values are less than 64. With the
exception of SIGABRT, these are also the Windows SIGXXX values.
...
.. table:: Summary of conformance requirements when \_AEABI\_PORTABILITY\_LEVEL != 0
  ...
  +==============+================+===========================================================================================================================================================================================================================================================================================================================+
  | assert.h     | Yes            | Must declare \_\_aeabi\_assert (`clibabi32-table3`_).                                                                                                                                                                                                                                                                     |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  ...
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | errno.h      | Yes            | errno is (\*\_\_aeabi\_errno()); , ERANGE, etc are link-time constants (`clibabi32-table4`_)                                                                                                                                                                                                                              |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | float.h      | No             |                                                                                                                                                                                                                                                                                                                           |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | inttypes.h   | No             |                                                                                                                                                                                                                                                                                                                           |
  ...
  | iso646.h     | No             |                                                                                                                                                                                                                                                                                                                           |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | limits.h     | Yes            | MB\_LEN\_MAX is a link-time constant (`clibabi32-table5`_).                                                                                                                                                                                                                                                               |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | locale.h     | Yes            | Must hide struct lconv and localeconv and declare struct \_\_aeabi\_lconv and \_\_aeabi\_localeconv (`clibabi32-table6`_, `clibabi32-table7`_). LC\_\* are link-time constants (`clibabi32-table8`_).                                                                                                                     |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | math.h       | Yes            | Must define HUGE\_VAL and similar using non-C89 means (e.g. C99 hex float notation) or provide suitably initialized const library members (`clibabi32-table9`_).                                                                                                                                                          |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | setjmp.h     | Yes            | Must declare jmp\_buf[] to preclude creating such objects. \_\_aeabi\_JMP\_BUF\_SIZE is a link-time constant (`clibabi32-table10`_).                                                                                                                                                                                      |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | signal.h     | Yes            | SIG\_\* are defined by the library (`clibabi32-table11`_); SIG\* are link-time const (`clibabi32-table12`_).                                                                                                                                                                                                              |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | stdarg.h     | No             |                                                                                                                                                                                                                                                                                                                           |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | stdbool.h    | No             |                                                                                                                                                                                                                                                                                                                           |
  ...
  | stdint.h     | No             |                                                                                                                                                                                                                                                                                                                           |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | stdio.h      | Yes            | Get/put macros must expand to function-calls; stdin, stdout, and stderr must expand to pointers, not addresses of FILE objects; FILE must be opaque. Some consensus constants must be defined as in `clibabi32-table13`_; other controlling values become link-time constants as defined in `clibabi32-table14`_.         |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | stdlib.h     | Yes            | MB\_CUR\_MAX must expand to the function call \_\_aeabi\_MB\_CUR\_MAX(); div\_t, ldiv\_t, EXIT\_\* must be declared as described in `clibabi32-table15`_.                                                                                                                                                                 |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | string.h     | No             |                                                                                                                                                                                                                                                                                                                           |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | time.h       | Yes            | time\_t ,clock\_t, and struct tm must be as specified in `clibabi32-table16`_. CLOCKS\_PER\_SEC must be a link-time constant.                                                                                                                                                                                             |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | wchar.h      | Yes            | wint\_t, WEOF, and mbstate\_t must be declared as specified in `clibabi32-table17`_.                                                                                                                                                                                                                                      |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  | wctype.h     | Yes            | wctype\_t and wctrans\_t must be opaque handles as specified in `clibabi32-table18`_.                                                                                                                                                                                                                                     |
  +--------------+----------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
